...
每项设置后都会回读校验（`IO` 用 `NtQueryInformationProcess(ProcessIoPriority)`，`THR` 用 `GetThreadPriority` 逐个线程复核）；只有真正生效才算 OK，否则标 `✗` 并在下一行给出错误码与原因。回读本身失败时同样按未生效处理，显示为 `✗nv`，不会被当成成功。
...
- `PRIO` / `AFF` / `ECO` / `IO` / `MEM` 是进程级设置；`THR` 只作用于执行瞬间已存在的线程（并逐个回读校验），之后新建的线程由进程优先级兜底。
